...
interface CarouselProps extends HTMLAttributes<HTMLDivElement> {
  ...
  ListItem: (
    data: (MovieShortData | Staff) & { ref: RefObject<HTMLLIElement> }
  ) => JSX.Element;
}
    ...
    <div className={styles.carousel} {...props}>
      ...
      <div className={styles.content}>
        <ul
          ...
          style={{
            right: liRef.current
              ? `${slide * liRef.current?.offsetWidth}px`
              : "0",
          }}
        >
          {list.map((e: MovieShortData | Staff, i: number) => (
            <ListItem {...e} key={i} ref={liRef} />
          ))}
        </ul>
      </div>
    </div>
